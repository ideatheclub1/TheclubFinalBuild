...
export const useMessaging = ({ conversationId, autoMarkAsRead = true }: UseMessagingProps = {}) => {
  ...
  const sendTypingIndicator = useCallback(() => {
    ...
    try {
      ...
      debugLogger.log('MESSAGING', 'TYPING_SENT', 'Typing indicator sent');
    } catch (err) {
  ...
  const stopTypingIndicator = useCallback(() => {
    ...
    try {
      ...
      debugLogger.log('MESSAGING', 'STOP_TYPING_SENT', 'Stop typing indicator sent');
    } catch (err) {
  ...
  useEffect(() => {
    ...
    debugLogger.log('MESSAGING', 'SETUP_REALTIME', 'Setting up real-time subscriptions');
    ...
    messageChannelRef.current = supabase
      ...
      .on(
        ...
        (payload) => {
          debugLogger.log('MESSAGING', 'REALTIME_MESSAGE', 'New message received', { messageId: payload.new.id });
      ...
      .on(
        ...
        (payload) => {
          debugLogger.log('MESSAGING', 'REALTIME_MESSAGE_UPDATE', 'Message updated', { messageId: payload.new.id });
      ...
      .on(
        ...
        (payload) => {
          debugLogger.log('MESSAGING', 'REALTIME_MESSAGE_DELETE', 'Message deleted', { messageId: payload.old.id });
      ...
      .subscribe((status) => {
        ...
        debugLogger.log('MESSAGING', 'REALTIME_STATUS', `Message subscription status: ${status}`);
      });
    ...
    return () => {
      debugLogger.log('MESSAGING', 'CLEANUP_REALTIME', 'Cleaning up real-time subscriptions');
