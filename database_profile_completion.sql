-- Add new fields to user_profiles table for better match finding
-- This script adds fields that are collected in the ProfileCompletionScreen

-- Add interests field (JSON array)
ALTER TABLE user_profiles 
ADD COLUMN IF NOT EXISTS interests TEXT[] DEFAULT '{}';

-- Add expertise field (JSON array)
ALTER TABLE user_profiles 
ADD COLUMN IF NOT EXISTS expertise TEXT[] DEFAULT '{}';

-- Add relationship_goals field (JSON array)
ALTER TABLE user_profiles 
...
-- Update RLS policies to allow users to update their own profile completion data
...
-- Update the dataService to include these new fields
-- Note: This is just for reference, the actual implementation is in the TypeScript files 