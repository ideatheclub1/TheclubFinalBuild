...
export default function ConversationScreen() {
  ...
  useEffect(() => {
    if (conversationId) {
      debugLogger.log('CONVERSATION', 'LOAD_MESSAGES', `Loading messages for conversation: ${conversationId}`);
  ...
  const handleSend = async () => {
    ...
    try {
      debugLogger.log('CONVERSATION', 'SEND_MESSAGE', `Sending message: ${text.substring(0, 50)}...`);
      
      const message = await sendMessage(text);
      
      if (message) {
        debugLogger.log('CONVERSATION', 'SEND_SUCCESS', 'Message sent successfully');
        // Auto-scroll to bottom
        setTimeout(() => {
          scrollViewRef.current?.scrollToEnd({ animated: true });
        }, 100);
      } else {
        debugLogger.log('CONVERSATION', 'SEND_FAILED', 'Failed to send message');
        Alert.alert('Error', 'Failed to send message');
      }
    } catch (err: any) {
      debugLogger.log('CONVERSATION', 'SEND_ERROR', 'Error sending message', err);
  ...
  const handleDeleteMessage = async (message: Message) => {
    try {
      debugLogger.log('CONVERSATION', 'DELETE_MESSAGE', `Deleting message: ${message.id}`);
      
      const success = await deleteMessage(message.id);
      
      if (success) {
        debugLogger.log('CONVERSATION', 'DELETE_SUCCESS', 'Message deleted successfully');
      } else {
        Alert.alert('Error', 'Failed to delete message');
      }
    } catch (err: any) {
      debugLogger.log('CONVERSATION', 'DELETE_ERROR', 'Error deleting message', err);
      Alert.alert('Error', `Failed to delete message: ${err.message || 'Unknown error'}`);
    }
  };

  const handleBack = () => {
    debugLogger.log('CONVERSATION', 'NAVIGATE_BACK', 'Navigating back to messages');
    router.back();
  };

  const handleCall = () => {
    debugLogger.log('CONVERSATION', 'CALL_ATTEMPT', `Attempting to call user: ${userName}`);
    Alert.alert('Voice Call', 'Voice calling feature coming soon!');
  };

  const handleVideoCall = () => {
    debugLogger.log('CONVERSATION', 'VIDEO_CALL_ATTEMPT', `Attempting video call with user: ${userName}`);
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          style: 'destructive',
          onPress: () => {
            debugLogger.log('CONVERSATION', 'CLEAR_CHAT', `Clearing chat: ${conversationId}`);
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          style: 'destructive',
          onPress: () => {
            debugLogger.log('CONVERSATION', 'BLOCK_USER', `Blocking user: ${userId}`);
