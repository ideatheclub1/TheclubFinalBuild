...
export default function ProfileScreen({ route }: ProfileScreenProps) {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (actualUserId && actualUserId !== '') {
          ...
          // Fetch posts
          const posts = await dataService.post.getPostsByUser(actualUserId, 20, 0, currentUser?.id);
  ...
  const handlePostPress = (post: Post) => {
    ...
    if (!currentUser) {
      debugLogger.error('NO_CURRENT_USER', 'No current user for like action');
      return;
    }

    try {
      debugLogger.info('LIKE_POST', 'Like post action initiated', { postId, userId: currentUser.id });
      
      // Call the actual like service
      ...
      if (success) {
        ...
        debugLogger.success('LIKE_POST_SUCCESS', 'Post like toggled successfully', { postId, userId: currentUser.id });
      } else {
        debugLogger.error('LIKE_POST_ERROR', 'Failed to toggle post like', { postId, userId: currentUser.id });
      }
    } catch (error) {
      debugLogger.error('LIKE_ACTION_ERROR', 'Like action error', { error: (error as Error).message, postId });
    }
  };
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    paddingBottom: 100, // Extra padding to ensure all content is scrollable
  },
  coverContainer: {
    height: 220,
    marginBottom: -70,
