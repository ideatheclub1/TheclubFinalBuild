...
export default function ProfileScreen({ route }: ProfileScreenProps) {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (actualUserId && actualUserId !== '') {
          ...
          debugLogger.log('NO_CONTENT_FETCH', 'No user ID available for content fetch');
          setUserPosts([]);
          setUserReels([]);
        }
  ...
  const handleMessages = async () => {
    console.log('🔵 HANDLE_MESSAGES: Button clicked!');
    debugLogger.log('PROFILE', 'MESSAGE_CLICK', 'Message button clicked');
    
    // Debug current state
    console.log('🔵 Current User:', currentUser?.id);
    console.log('🔵 Target User:', user?.id);
    console.log('🔵 dataService.message exists:', !!dataService.message);
    
    if (!currentUser?.id || !user?.id) {
      console.log('❌ Missing user IDs - currentUser:', currentUser?.id, 'user:', user?.id);
      debugLogger.error('PROFILE', 'MISSING_USER_DATA', 'Missing user IDs for conversation');
      Alert.alert('Error', 'Unable to start conversation - missing user data');
      return;
    }

    if (user.id === currentUser.id) {
      console.log('❌ Cannot message self');
      debugLogger.log('PROFILE', 'SELF_MESSAGE_ATTEMPT', 'User tried to message themselves');
    ...
    try {
      ...
      debugLogger.process('PROFILE', 'START_CONVERSATION', `Starting conversation with user: ${user.id}`);
      
      // Check if dataService.message exists
      if (!dataService.message || !dataService.message.getConversations) {
        console.log('❌ dataService.message not available');
        debugLogger.error('PROFILE', 'SERVICE_ERROR', 'Message service not available');
        Alert.alert('Error', 'Messaging service not available');
        return;
      }
      ...
      if (existingConversation) {
        ...
        debugLogger.success('PROFILE', 'EXISTING_CONVERSATION', `Found existing conversation: ${existingConversation.id}`);
        router.push({
        ...
        debugLogger.process('PROFILE', 'CREATING_CONVERSATION', 'Creating new conversation');
        
        // Create new conversation
        const conversationId = await dataService.message.createConversation([currentUser.id, user.id]);
        console.log('🔵 New conversation ID:', conversationId);
        
        if (conversationId) {
          debugLogger.success('PROFILE', 'NEW_CONVERSATION', `Created new conversation: ${conversationId}`);
          router.push({
            pathname: '/conversation',
            params: { 
              conversationId,
          ...
          debugLogger.error('PROFILE', 'CREATE_CONVERSATION_FAILED', 'Failed to create conversation');
          Alert.alert('Error', 'Failed to create conversation');
        }
      }
    } catch (error) {
      console.log('❌ Error in handleMessages:', error);
      debugLogger.error('PROFILE', 'MESSAGE_ERROR', 'Failed to start conversation', error);
