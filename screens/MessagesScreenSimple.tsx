...
export default function MessagesScreen() {
  ...
  const { conversations, loading, error, isConnected, loadConversations } = useMessaging();
  ...
  const handleConversationPress = (conversation: Conversation) => {
    debugLogger.log('CONVERSATION_PRESS', `Opening conversation: ${conversation.id}`);
  ...
  const handleMarkAsRead = (conversation: Conversation) => {
    ...
    debugLogger.log('MARK_AS_READ', `Marking conversation as read: ${conversation.id}`);
  };
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Archive', style: 'destructive', onPress: () => {
          debugLogger.log('ARCHIVE', `Archiving conversation: ${conversation.id}`);
  ...
  const handleNewMessage = () => {
    debugLogger.log('NEW_MESSAGE', 'Opening new message screen');
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      debugLogger.log('REFRESH_ERROR', 'Failed to refresh conversations', err);
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handleSearch = (text: string) => {
    setSearchQuery(text);
    debugLogger.log('SEARCH', `Searching conversations: ${text}`);
  };
        ...
        {!isConnected && (
          <View style={styles.connectionBanner}>
            <Text style={styles.connectionText}>
              ⚠️ Connection issues - Messages may not update automatically
            </Text>
          </View>
        )}
